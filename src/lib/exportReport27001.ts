...
export function exportReport27001({ meta, statuses, notes, findings }: ExportArgs) {
  const counts: Record<Status, number> = { pending: 0, conformant: 0, ofi: 0, minor: 0, major: 0, na: 0 };
  for (const it of ISO27001_ALL_ITEMS) counts[statuses[it.clause] ?? "pending"]++;
  ...
  const findingItems = ISO27001_ALL_ITEMS.filter((it) => {
...
<title>ISMS Audit Report — ${esc(meta.organization || "Organization")}</title>
<style>
  @page { margin: 22mm; }
  * { box-sizing: border-box; }
  ...
  <h1>Information Security Management System Audit<span class="accent">.</span></h1>
  ...
  <table class="meta">
    ...
    <tr><td>Standard</td><td>ISO/IEC 27001:2022 (clauses 4–10) + Annex A controls referenced via SoA</td></tr>
    <tr><td>Report generated</td><td>${today}</td></tr>
  </table>

  <h2>1. Executive Summary</h2>
  <p>The audit covered ${total} ISO/IEC 27001:2022 management-system clauses. Overall conformity rate is
  ...
  <h2>2. Annex A control themes (referenced via SoA)</h2>
  <table>
    <thead><tr><th style="width:80px;">Theme</th><th>Name</th><th style="width:80px;">Controls</th><th>Examples</th></tr></thead>
    <tbody>
      ${ANNEX_A_THEMES.map(t => `<tr>
        <td><strong>${t.code}</strong></td><td>${esc(t.name)}</td>
        <td>${t.count}</td><td>${esc(t.examples)}</td></tr>`).join("")}
    </tbody>
  </table>

  <h2>3. Clause Coverage</h2>
  <table>
    <thead><tr><th style="width:80px;">Clause</th><th>Title</th><th style="width:130px;">Status</th></tr></thead>
    <tbody>
      ${ISO27001_GROUPS.flatMap(g => g.items.map(it => {
  ...
  ${findingItems.length === 0 ? "<p>No nonconformities or opportunities for improvement were raised.</p>" : findingItems.map((it, i) => {
    ...
    return `<div class="finding">
      ...
      <table>
        <tr><td style="width:30%;background:#f3eee2;font-weight:600;">Title</td><td>${esc(it.title)}</td></tr>
        <tr><td style="background:#f3eee2;font-weight:600;">Audit question</td><td>${esc(it.question)}</td></tr>
        <tr><td style="background:#f3eee2;font-weight:600;">Expected evidence</td><td>${esc(it.evidence)}</td></tr>
        <tr><td style="background:#f3eee2;font-weight:600;">Evidence reviewed / notes</td><td>${esc(notes[it.clause] || "—")}</td></tr>
        <tr><td style="background:#f3eee2;font-weight:600;">Finding statement</td><td>${esc(findings[it.clause] || "—")}</td></tr>
      </table>
    </div>`;
  }).join("")}
  ...
  <h2>5. Approval</h2>
  <table class="meta">
    <tr><td>Prepared by (Lead Auditor)</td><td>${esc(meta.leadAuditor || "—")} — ${today}</td></tr>
    <tr><td>Reviewed by (CISO / ISMS Manager)</td><td>_________________________ Date: __________</td></tr>
    <tr><td>Acknowledged by (Auditee)</td><td>_________________________ Date: __________</td></tr>
  </table>

  <footer>Confidential — Internal Use Only · Generated by Conformia · ISO/IEC 27001:2022 audit workspace</footer>

  <script>window.onload = () => setTimeout(() => window.print(), 400);</script>
</body></html>`;

  const w = window.open("", "_blank");
  if (!w) { alert("Pop-up blocked. Please allow pop-ups to export the report."); return; }
