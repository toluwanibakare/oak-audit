            ...
            <table className="w-full text-xs">
              <thead className="bg-muted/40 border-b border-border">
                <tr className="text-left">
                  ...
                  <th className="py-2.5 px-3 annotation font-medium">Standard(s)</th>
              ...
              <tbody>
                {teams.length === 0 && (
                  <tr><td colSpan={7} className="py-10 text-center text-muted-foreground">No teams yet.</td></tr>
                )}
                {teams.map((t) => (
                    ...
                    <td className="py-2.5 px-3">
                      {t.standards ? (
                        <div className="flex flex-wrap gap-1">
                          {t.standards.split(", ").map((s) => <WBadge key={s} tone="outline">{s}</WBadge>)}
                        </div>
      ...
      <div className="bg-card border border-border rounded-lg w-full max-w-lg shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 grid grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto">
          ...
          <label className="flex flex-col gap-1">
            ...
            {standards.length === 0 ? (
              <div className="h-9 px-2 rounded-md border border-dashed border-amber-500/40 bg-amber-500/5 flex items-center text-[11px] text-amber-600 dark:text-amber-400">
                No active standards. Enable them in Settings first.
              </div>
            ) : (
              <div className="max-h-24 overflow-y-auto rounded-md border border-input bg-muted p-1 space-y-0.5">
                {standards.map((s: any) => {
                  const checked = selectedStandards.includes(s.code);
                  return (
                    <label key={s.id} className={`flex items-center gap-2 px-2 py-1 rounded cursor-pointer text-xs hover:bg-muted/80 ${checked ? "bg-primary/10" : ""}`}>
                      <input type="checkbox" checked={checked} onChange={() => {
                        setSelectedStandards(checked ? selectedStandards.filter((c) => c !== s.code) : [...selectedStandards, s.code]);
          ...
          <label className="flex flex-col gap-1">
            ...
            {users.length === 0 ? (
              <div className="h-9 px-2 rounded-md border border-dashed border-amber-500/40 bg-amber-500/5 flex items-center text-[11px] text-amber-600 dark:text-amber-400">
                No users yet. Create one in User Management &gt; Users first.
              </div>
            ) : (
